...
using std::vector;
void usage();
void send2supervisor(const string&, const string&, const string&, const string&,
...
try {
  ...
  p.add<std::string>("cmd", 'c',
                     "command to send servers (start|stop|save|load|status)",
                     true);
  p.add<std::string>(
      "server", 's',
      "server exec file of learning machine (jubaclassifier, ...)", true);
  p.add<std::string>("name", 'n', "learning machine name", true);
  p.add<std::string>("type", 't', "learning machine type (classifier, ...)",
                     true);
  p.add<unsigned int>(
      "num", 'N',
  ...
  p.add<std::string>("datadir", 'D',
                     "[start] directory to load and save models", false,
                     "/tmp");
  p.add<std::string>("logdir", 'L',
                     "[start] directory to output logs (instead of stderr)",
                     false, "");
  p.add<int, cmdline::range_reader<int> >(
      "loglevel", 'E', "[start] verbosity of log messages", false, google::INFO,
  ...
  p.add<int>("interval_count", 'I', "[start] mix interval by update count",
             false, 512);
...
int do_request(const string& cmd, const string& name, const string& ip_port,
  ...
  cout << "sending " << cmd << " / " << name << " to " << ip_port << "..."
      << std::flush;
  int r;
  if (cmd == "start") {
...
void send2supervisor(const string& cmd, const string& type, const string& name,
...
void show(jubatus::common::lock_service& z, const string& path,
          const string& name) {
  vector < string > list;
  z.list(path, list);
